...
class Particale {
  ...
  init() {
    ...
    else this.radius >= 2 ? (this.radius = this.radius) : (this.radius = 2);
  ...
  drawPic() {
    ...
    if (imgObj.width > imgObj.height) {
      ...
      imgObj.width = this.warp.width * 0.5;
      imgObj.height = imgObj.width * ImgScale;
    } else {
      ImgScale = imgObj.width / imgObj.height;
      imgObj.height = this.warp.height * 0.7;
      imgObj.width = imgObj.height * ImgScale;
    }
    //绘制图片到canvas
    this.ctx.drawImage(
      imgObj,
      this.warp.width / 2 - imgObj.width / 2,
      this.warp.height / 2 - imgObj.height / 2,
      imgObj.width,
      imgObj.height
    );
  }
  ...
  drowDot(dot) {
    ...
    this.ctx.save();
    this.ctx.beginPath();
    this.ctx.fillStyle = `rgba(${Math.floor(dot.ir)}, ${Math.floor(
      dot.ig
    )}, ${Math.floor(dot.ib)}, ${dot.ia})`;
    this.ctx.arc(
      this.warp.width / 2 + (dot.ix - this.warp.width / 2) * scale,
      this.warp.height / 2 + (dot.iy - this.warp.height / 2) * scale,
      this.radius * scale,
      0,
      Math.PI * 2
    );
