 ...
 * @LastEditTime: 2023-06-29 04:28:59
...
.Forum-content-a:hover .b2timeago-a {
  display: inline;
}
...
<template>
  <div class="Forum">
    ...
    <div class="Forum-content">
      <div class="Forum-content-a">
        ...
        <div class="Forum-content-other">
          ...
          <div class="iconbutton">
            <button class="reply iconbutton" @click="Tworeply(item)">
              <svg
                t="1687894334652"
                class="icon"
              ...
              <div style="margin: -1.2px 0 0 2px">{{ item.reply }}</div>
            </button>
      ...
      <!-- 回复的回复框 -->
      <div>
        <div class="Forum-content-repling" v-if="replytext1show">
          <!-- 回复文本框 -->
          <div>
            <div class="commentform">
              <el-input v-model="replytext1" placeholder="您的看法" />
            </div>
          </div>
          <!-- 回复表情发布框 -->
          <div
          ...
          >
            ...
            <div style="margin-right: 1rem">
              <button class="but" @click="replytext1cancel">取消</button>
              <button class="but send" @click="replytext1send(item)">
                发送
              </button>
            </div>
          </div>
      ...
      <!-- 回复的回复 -->
      <div class="Forum-content-reply">
        <ul>
          <li v-for="(item, index) in replyTwo" :key="index">
            <!-- 谁回复 -->
            <ForumTwo
              :item="item"
              :fatherid="fatherid"
...
import { getReplyTwo, getNickname, getAuthorId } from "../../../api/artcate";
import { addReplyTwo, deleteReply } from "../../../api/article";
...
const fatherid = props.item.id;
const userStore = useUserStore();
const nickname = ref(userStore.nickname);
//回复函数
const replytext1 = ref("");
const replytext1show = ref(false);
//获取回复的二级内容
const replyTwo = ref();
//接口函数
const getreplyTwo = async (uid) => {
  const res = await getReplyTwo({
    id: uid,
  });
  console.log(uid);
  console.log(res.data);
  const userIds = res.data.map((item) => item.user_id); // 获取所有用户的id
  const otherIds = res.data.map((item) => item.other_id); // 获取所有用户的id
  const users = await getNickname(userIds); // 查询所有回复用户的昵称
  const others = await getNickname(otherIds); // 查询别人用户的昵称
  replyTwo.value = res.data.map((item) => {
    const user = users.data.find((u) => u.id === item.user_id); // 查找当前回复的用户
    const other = others.data.find((u) => u.id === item.other_id); // 查找当前回复的用户
    const imgurl =
...
const Tworeply = (item) => {
  getreplyTwo(item.id);
  replytext1show.value = !replytext1show.value;
};
//取消清空value
const replytext1cancel = () => {
  replytext1show.value = false;
  replytext1.value = "";
};
//发送回复任务
const replytext1send = async (item) => {
  ...
  if (!localStorage.getItem("token") || !id) {
    ...
    const res = await getAuthorId({
      name: item.user_id,
    });
    //被回复人
    // const otherId = res.data.id;
    //发送人id
